...
- (void)callFailed:(OAServiceTicket *)aTicket withError:(NSError *)error {
	...
	OAProblem *problem = [OAProblem problemWithResponseBody:_ticket.body];
	if (problem) {
		[_delegate call:self failedWithProblem:problem];
	} else {
		[_delegate call:self failedWithError:error];
	}
}
...
- (void)callFinished:(OAServiceTicket *)aTicket withData:(NSData *)data {
	...
	if (_ticket.didSucceed) {
//		NSLog(@"Call body: %@", ticket.body);
		[_delegate performSelector:_finishedSelector withObject:self withObject:_ticket.body];
	} else {
//		NSLog(@"Failed call body: %@", ticket.body);
		[self callFailed:_ticket withError:nil];
	}
}
...
//{
//	_delegate = aDelegate;
//	_finishedSelector = finished;
//
//	_request = [[OAMutableURLRequest alloc] initWithURL:_url
//											  consumer:consumer
//												token:token
//												 realm:realm
//									 signatureProvider:nil];
//	if(_method) {
//		[_request setHTTPMethod:_method];
//	}
//
//	if (self.parameters) {
//		[_request setParameters:self.parameters];
//	}
//	if (self.files) {
//		for (NSString *key in self.files) {
//			[_request attachFileWithName:@"file" filename:NSLocalizedString(@"Photo.jpg", @"") data:[self.files objectForKey:key]];
//		}
//	}
